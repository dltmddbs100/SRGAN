...
for epoch_i in range(0, args['max_epochs']):
  ...
  for i,batch in enumerate(test_loader):
    ...
    valing_results = {'mse': 0, 'psnr': 0}

    with torch.no_grad():
      sr=netG(lr)

    batch_mse = ((sr - hr) ** 2).data.mean()
    valing_results['mse'] += batch_mse * args['batch_size']
    valing_results['psnr'] += 10 * log10((hr.max()**2) / (valing_results['mse'] / args['batch_size']))

    print(f"\rTest Batch {i+1}/{total_batch} , mse : {valing_results['mse']/(i+1):.6f}, psnr : {valing_results['psnr']/(i+1):.6f}", end='')
